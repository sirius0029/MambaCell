...
TOKEN_DICTIONARY_FILE = current_dir / "token_dictionary.pkl"
...
class MambaCellPreCollator(SpecialTokensMixin):
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, features_1, features_2):
        ...
        contrastive_1 = features_1["contrastive"]
        contrastive_2 = features_2["contrastive"]
        
        sim_matrix = torch.mm(contrastive_1,  contrastive_2.T) / self.temperature
        
        
        labels = torch.arange(contrastive_1.size(0), 
                            device=contrastive_1.device,
                            dtype=torch.long)
        
      
        loss_12 = self.cross_entropy(sim_matrix, labels)
        loss_21 = self.cross_entropy(sim_matrix.T, labels)
        
        return (loss_12 + loss_21) / 2

class DataCollator(DataCollatorForLanguageModeling):
    def __init__(self, tokenizer, mlm=True, mlm_probability=0.15, task="joint"):
    ...
    def __call__(self, examples):
        ...
        if mask_positions.sum() == 0: 
                self.mlm_probability = min(0.3, self.mlm_probability + 0.05)
                return self.__call__(examples)
                
        batch["mask_positions"] = mask_positions.to(torch.bool)
        
        if self.task == "cl":
         
            view1 = {k: v.clone() for k, v in batch.items() if k != "mask_positions"}
            view2 = super().__call__(examples)  
            
          
            batch.update({
                "input_ids_1": view1["input_ids"],
                "attention_mask_1": view1["attention_mask"],
                "input_ids_2": view2["input_ids"],
                "attention_mask_2": view2["attention_mask"]
            })
            
        
            cross_mask = (view1["input_ids"] == self.tokenizer.mask_token_id) | \
                        (view2["input_ids"] == self.tokenizer.mask_token_id)
            batch["cross_mask"] = cross_mask.to(torch.bool)
            ...
            batch["input_ids_1"] = view1["input_ids"]
            batch["attention_mask_1"] = view1["attention_mask"]
            batch["input_ids_2"] = view2["input_ids"]
            batch["attention_mask_2"] = view2["attention_mask"]
            batch["cross_mask"] = cross_mask.to(torch.bool)
...
class MambaCellPretrainer(Trainer):
    def __init__(self, *args, alpha=0.7, beta=0.3, **kwargs):
        # Get the task parameter 
        self.task = kwargs.pop("task", "joint")
        self.loss_weights = {
            "mgm": alpha, 
            "cl": beta
        }
        
        # Get data_collator and token_dictionary
        data_collator = kwargs.get("data_collator", None)
        token_dictionary = kwargs.pop("token_dictionary")

        # If data_collator is not provided, create a default one
        if data_collator is None:
            precollator = MambaCellPreCollator(token_dictionary=token_dictionary)
            data_collator = DataCollator(
                tokenizer=precollator, mlm=True, mlm_probability=0.15,task=self.task,
            )
            kwargs["data_collator"] = data_collator

        # Load example_lengths_file
        ...
        if example_lengths_file:
            ...
            raise Exception(
            "example_lengths_file is required;"
        )
        super().__init__(*args, **kwargs)
    

    # Override compute_loss method to support dynamic loss
    def compute_loss(self, model, inputs, return_outputs=False,**kwargs):
        """
        Select different loss functions based on the task parameter.
        """
        labels = inputs.pop("labels", None)  # Get labels
        mask_positions = inputs.pop("mask_positions", None)  # Get mask_positions (for MPG)
        losses = {}
        
        if mask_positions is None:
                   print("warning:void mask")
        if self.task in ["mgm","joint"]:
            outputs = model(
        ...
        if self.task in ["mgm", "joint"]:
            # Masked Pretraining Classification (MP-C) uses cross-entropy loss
            loss_fn = nn.CrossEntropyLoss()
            loss = loss_fn(outputs["mlc_logits"].view(-1, outputs["mlc_logits"].size(-1)), labels.view(-1))
            losses["mgm"] =loss
            
        if self.task in ["cl", "joint"]:
        ...
        if self.task not in ["mgm,", "cl", "joint"]:
            raise ValueError(f"Unsupported task: {self.task}")
            
        if self.task == 'joint':
            total_loss = sum(w * losses[k] for k, w in self.loss_weights.items())
        
        return total_loss if self.task == 'joint' else loss

    # Modify LengthGroupedSampler logic (keep unchanged)
...
'''
class GeneformerPretrainer(MambaTrainer):
    def __init__(
        self,
        model,
        train_dataset,
        ...
        if isinstance(token_dictionary, dict):
            with open("tokenizer.json", "w") as f:
                json.dump(token_dictionary, f,default=custom_json_serializer)
            self.token_dictionary = AutoTokenizer.from_pretrained("tokenizer.json")
        else:
            self.token_dictionary = token_dictionary

        if self.token_dictionary.mask_token is None:
            self.token_dictionary.mask_token = "[MASK]"

        # Load example lengths (for length-based sampling)
        with open(example_lengths_file, "rb") as f:
            self.example_lengths = pickle.load(f)

        # Initialize data collator
        self.data_collator = DataCollatorForLanguageModeling(
            tokenizer=self.token_dictionary, mlm=True, mlm_probability=0.15
        )
    ...
    def get_dataloader(self, dataset):
        """Override to use length-based sampling."""
    ...
    def _get_train_sampler(self):
        """Custom sampler for length-based grouping."""
